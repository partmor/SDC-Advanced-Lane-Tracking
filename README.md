...
Each group of image plane coordinates is paired with their theoretical position, which happens to always be the same `objp` since the same real-world object is being evaluated.

The (image points, object points) tuples are passed to the `cv2.calibrateCamera()` method that returns the camera's distortion parameters.

Given the latter parameters `cv2.undistort()` allows to undistort any image taken by that camera. Applying this transform on the test image:
...
+ Absolute **sobel** thresholding in the **X direction** of the **L channel** picks very well edges of white and yellow lines where the basic color channels seem to fail.
...
It is worth pointing out that the YUV color space provides very similar results to the Lab: 
...
The next step in the pipeline is to catalogue the pixels in the warped binary mask as belonging to the left or right lane.

The lanes are scanned using a **vertical-sliding window**. Consider a static window of the image that covers a vertical portion of the image and spans along the whole horizontal axis. In this window, a histogram of the binary image along the horizontal direction will present peaks in aeras with high density of highlighted pixels. It is reasonable to affirm that the 2 largest peaks are due to the lanes.

A bounding box with a given width (the height is equal to the window's) is defined around the peaks, and all the pixels that lie inside it are labeled as belonging to the left or right lane, depending if the box is located on the left or right half of the image.

This process is repeted for subsequent and non-overlapping windows, ending up with a collection of pixels tagged as left/right.

With the coordinates of the pixels in the rectfied image, a second degree polynomial is fitted to each lane.

This methodology is encapsulated in `aux_fun.find_lanes_sliding_window_hist()` and produces:
...
The **radius of curvature** of a line is defined in terms of its first and second spatial derivatives. Hence, the curvature of the fitted lines in the warped image can be immediately calculated since the  expression *x = f(y)* is known for both.

However, this radius is calculated based on pixel values in the warped space. The U.S. regulations for lane dimensions allow to define an aproximate conversion factor from pixel space to real space: 30/720 [meters/pixel] in the *y* dimension, and 3.7/700 [meters/pixel] in the *x* dimension.

The method `aux_fun.calculate_radius_in_meters()` implements the form factor correction and calculates the radius of curvature in meters, receiving the fitted line coefficients from the pixel space.

The **offset** is defined as the horizontal distance between the lane midpoint and image midpoint, evaluated in the bottom edge of the image, positive when the vehicle (midpoint of the image) is on the right of the lane center.
